        ...
        if 'ZISSHUDBUNIq' not in message['content']: #The state, a special string to identify which type of url it is.
            ...
            if str(r) == '<Response [200]>':
                content = "Please click the link below to log in and authorise Joinme:\n {} \nAnd please copy and " \
                          "send the url shown in browser after clicking accept. " \
                          "Don't forget to @-mention me!".format(r.url)

            else:
                content = "Please check if you have entered the correct API key and callback URL " \
                          "in joinme.conf."
        else:
            ...
            payload = {"client_id": self.config_info['key'], "client_secret": self.config_info['secret'],
                       "code": authorization_code[0], "redirect_uri": self.config_info['callback_url'],
