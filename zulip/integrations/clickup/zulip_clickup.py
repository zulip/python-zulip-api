...
ClickUpAPIQueryT: TypeAlias = Dict[str, Union[str, List[str]]]
...
class ClickUpAPIHandler:
    ...
    def make_clickup_request(
        self, endpoint: str, query: ClickUpAPIQueryT, method: str
    ) -> Optional[Dict[str, Any]]:
        base_url = "https://api.clickup.com/api/v2/"
        api_endpoint = urljoin(base_url, endpoint)

        if endpoint == "oath/token":
    ...
    def initialize_access_token(self, auth_code: str) -> None:
        ...
        query: ClickUpAPIQueryT = {
            "client_id": self.client_id,
            "client_secret": self.client_secret,
            "code": auth_code,
        }
        data = self.make_clickup_request(endpoint, query, "POST")
    ...
    def create_webhook(self, end_point: str, events: List[str]) -> Dict[str, Any]:
        ...
        """
        endpoint = f"team/{self.team_id}/webhook"
        query: ClickUpAPIQueryT = {
            "endpoint": end_point,
            "events": events,
        }
    ...
    def get_webhooks(self) -> Dict[str, Any]:
        ...
        """
        endpoint = f"team/{self.team_id}/webhook"
        data = self.make_clickup_request(endpoint, {}, "GET")
        if data is None:
